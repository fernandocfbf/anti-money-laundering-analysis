...
date_input = st.sidebar.date_input(label="Period of analysis", format="DD/MM/YYYY")
selected_account_id = st.sidebar.selectbox('Choose a account ID to analyse:', set(transactions_df['sender']))
max_depth = st.sidebar.number_input("Select network max depth", value=2, placeholder="Type a number...")
max_nodes = st.sidebar.number_input("Select network max nodes", max_value=300, value=50, placeholder="Type a number...")
physics = st.sidebar.checkbox('enable physics', value=False)
